...
public record TrainRow
{
    ...
    // Rolling features (kg/day historik)
...
{
    public float Score { get; set; } // predicted kg/day
}
...
public record Recommendation(
    ...
    string CustomerName,
    string Skabelonnr,
    int RecommendedContainerL,
    int RecommendedFrequencyDays,
    double ExpectedFill,
    int? CurrentContainerL,
    string Note
);
...
{
    // === Business constants ===
    const double DensityKgPerL = 0.13; // restaffald (kg/L)
    static readonly int[] AllowedContainers = { 120, 240, 660, 1100, };

    // Search space for frequency
    ...
    // Safety buffer (reduce overfill risk)
    // 0.0 = off, 0.5 = mild, 1.0 = more conservative
    const double SafetyK = 0.75;

    // Cost weights (tune these)
    // Overfill is worst => bigger penalty
    const double PenaltyOver = 1200.0;
    const double PenaltyUnder = 250.0;

    // Pickup cost (pushes toward fewer pickups)
    ...
    public static void Main()
    {
        ...
        // 2) Varenummer -> KapacitetL (kun L + tilladte størrelser)
        var kapDict = kap
        ...
        // 3) Skabelonnr -> current container size (via Kørselsordrer + kapacitet)
        var skabelonToCurrentCap = koersel
            .Where(k => !string.IsNullOrWhiteSpace(k.Skabelonnr) && !string.IsNullOrWhiteSpace(k.Varenummer))
            .Where(k => kapDict.ContainsKey(k.Varenummer))
            .GroupBy(k => k.Skabelonnr.Trim())
            .ToDictionary(g => g.Key, g => (int?)kapDict[g.First().Varenummer]);

        // 4) Daily aggregation (KG)
        var daily = modtag
        ...
        // 5) Build training rows + rolling features
        ...
        // 6) Clip extreme outliers (stabiliser RMSE)
        ...
        // Pipeline: OneHot Skabelonnr + concat numeric + FastTree regression
        var basePipe =
        ...
        // 10) Residual STD per skabelonnr (from TRAIN) -> safety buffer
     var residualStdBySkabelon = ComputeResidualStdBySkabelonnr(ml, bestModel, trainRows);

        // 11) Recommendation accuracy on TEST using cost-based chooser + safety buffer
        ...
        for (int i = 0; i < testRows.Count; i++)
        {
            ...
            var (containerL, freqDays, expectedFill, note) = ChooseBestComboCostBased(predSafe);

            var trueKgDay = testRows[i].LabelKgPerDay;
            double fillTrue = (trueKgDay * freqDays / DensityKgPerL) / containerL;
        ...
        // 12) Latest per skabelon -> recommendations
        var latestBySkabelon = rows
            .GroupBy(r => r.Skabelonnr)
            .Select(g => g.OrderByDescending(x => x.Date).First())
            .ToList();

        var recs = Recommend(ml, bestModel, latestBySkabelon, skabelonToCurrentCap, residualStdBySkabelon);

        // 13) Interactive console search
        RunInteractiveSearch(recs);
    }
    ...
    static List<Recommendation> Recommend(
        MLContext ml, ITransformer model,
        List<TrainRow> latest,
        Dictionary<string, int?> skabelonToCurrentCap,
        Dictionary<string, double> residStdBySkabelon)
    {
        var inputs = latest.Select(ToModelInputNoLabel).ToList();
        ...
        {
            var r = latest[i];
            var sk = r.Skabelonnr.Trim();

            var predKgDay = preds[i].Score;
            var residStd = residStdBySkabelon.TryGetValue(sk, out var s) ? s : residStdBySkabelon["__GLOBAL__"];

            var predSafe = predKgDay + (float)(SafetyK * residStd);

            var (containerL, freqDays, expectedFill, note) = ChooseBestComboCostBased(predSafe);

            skabelonToCurrentCap.TryGetValue(sk, out var currentCap);

            outList.Add(new Recommendation(
                r.CustomerNo, r.CustomerName, r.Skabelonnr,
                containerL, freqDays, expectedFill,
                currentCap,
                note
            ));
        }
    ...
    static (int containerL, int freqDays, double expectedFill, string note) ChooseBestComboCostBased(double kgPerDayPredSafe)
    {
        ...
        foreach (var capL in AllowedContainers)
        {
            ...
            {
                double fill = (kgPerDayPredSafe * freq / DensityKgPerL) / capL;

                // penalties for being outside [0.8,1.0]
                double under = Math.Max(0, TargetMinFill - fill);
                double over  = Math.Max(0, fill - TargetMaxFill);

                // pickup cost: fewer pickups better -> cost proportional to pickups/year approx
                // pickups/year ~ 365/freq
                double pickupsPerYear = 365.0 / freq;

                double cost = (PenaltyUnder * under) + (PenaltyOver * over) + (PickupCostPerPickup * pickupsPerYear);

                // Tie-breakers: prefer fewer pickups, then smaller container
                bool better = cost < bestCost
                              || (Math.Abs(cost - bestCost) < 1e-9 && freq > bestF)
                              || (Math.Abs(cost - bestCost) < 1e-9 && freq == bestF && capL < bestC);
                ...
                if (better)
                {
                    ...
                    bestNote = (fill >= TargetMinFill && fill <= TargetMaxFill)
                        ? ""
                        : (fill < TargetMinFill ? "Outside target (underfilled) - best compromise" : "Outside target (overfilled risk) - best compromise");
                }
            }
        }

        return (bestC, bestF, bestFill, bestNote);
    }

    // =========================
    // Residual std (for safety buffer)
    // =========================
    static Dictionary<string, double> ComputeResidualStdBySkabelonnr(MLContext ml, ITransformer model, List<TrainRow> trainRows)
    {
        ...
        {
            // only compute per-skabelon if enough data; else fallback to global
    ...
    // =========================
    // Build training rows w/ rolling features
    // =========================
    static List<TrainRow> BuildTrainingRowsWithRolling(List<PickupDaily> daily)
    {
        ...
        foreach (var grp in daily.GroupBy(d => d.Skabelonnr))
        {
            ...
            // store previous kg/day values to compute rolling stats
            ...
            for (int i = 1; i < list.Count; i++)
            {
                ...
                if (days <= 0) continue;
                if (days > 180) continue; // optional: ignore huge gaps

                double kgPerDay = cur.CollectedKg / days;

                // rolling features from history BEFORE adding current kg/day
                ...
                // if too little history, fall back to current estimate (better than 0)
        ...
        // simple linear trend (slope) over last k points: x=0..n-1, y=kg/day
    ...
    // Clip labels to reduce extreme RMSE impact
    ...
    // =========================
    static void RunInteractiveSearch(List<Recommendation> recs)
    {
        var byCustomerNo = recs
            .Where(r => !string.IsNullOrWhiteSpace(r.CustomerNo))
            .GroupBy(r => r.CustomerNo.Trim())
            .ToDictionary(g => g.Key, g => g.ToList());

        var bySkabelon = recs
            .Where(r => !string.IsNullOrWhiteSpace(r.Skabelonnr))
            .GroupBy(r => r.Skabelonnr.Trim())
            .ToDictionary(g => g.Key, g => g.ToList());

        Console.WriteLine("\n--- SØGNING ---");
        Console.WriteLine("Skriv kundenr eller kundenavn (eller Skabelonnr). Skriv 'exit' for at stoppe.");

        while (true)
        {
            Console.Write("\nSøg: ");
            var q = (Console.ReadLine() ?? "").Trim();
            if (q.Equals("exit", StringComparison.OrdinalIgnoreCase)) break;
            if (q.Length == 0) continue;

            if (byCustomerNo.TryGetValue(q, out var hitsNo)) { PrintRecs(hitsNo); continue; }
            if (bySkabelon.TryGetValue(q, out var hitsSk)) { PrintRecs(hitsSk); continue; }

            var qLower = q.ToLowerInvariant();
            var hitsName = recs.Where(r => (r.CustomerName ?? "").ToLowerInvariant().Contains(qLower)).ToList();

            if (hitsName.Count == 0) Console.WriteLine("Ingen match.");
            else
            {
                var limited = hitsName.Take(25).ToList();
                Console.WriteLine($"Fandt {hitsName.Count} (viser {limited.Count}):");
                PrintRecs(limited);
            }
        }
    }

    static void PrintRecs(List<Recommendation> list)
    {
        foreach (var grp in list.GroupBy(r => (r.CustomerNo, r.CustomerName)))
        {
            Console.WriteLine($"\nKunde: {grp.Key.CustomerNo} - {grp.Key.CustomerName}");
            foreach (var r in grp.OrderBy(x => x.Skabelonnr))
            {
                var cur = r.CurrentContainerL is null ? "" : $" | nuværende: {r.CurrentContainerL}L";
                var note = string.IsNullOrWhiteSpace(r.Note) ? "" : $" | NOTE: {r.Note}";
                Console.WriteLine($"  {r.Skabelonnr}: {r.RecommendedContainerL}L hver {r.RecommendedFrequencyDays} dage | expectedFill={r.ExpectedFill:P0}{cur}{note}");
            }
        }
    }
    ...
    static List<ModtagRow> ReadModtagelser(string path)
    {
        ...
        int? cCustNo   = TryFindCol(t, "Kundenr", "KundeNr", "CustomerNo", "DebitorNr", "Debitor");
        int? cCustName = TryFindCol(t, "Kundenavn", "KundeNavn", "CustomerName", "Customer");
        ...
        foreach (DataRow r in t.Rows)
        {
            ...
            list.Add(new ModtagRow(sk, dt, en, antal, custNo, custName));
        }
        return list;
    }
