...
public async Task<List<PickupDailyDb>> LoadDailyPickupsForCustomerAsync(DateTime fromDate, DateTime toDate, int customerKey)
{
    fromDate = fromDate.Date;
    toDate = toDate.Date;
    if (toDate < fromDate) (fromDate, toDate) = (toDate, fromDate);
    ...
    var raw = await _db.StenaReceipts.AsNoTracking()
        .Where(r => r.Unit == "KG"
                    ...
                    && residualOrders.Contains(r.PurchaseOrderNumber.Value)
                    && r.CustomerKey != null
                    && r.CustomerKey.Value == customerKey)
        .Select(r => new
        {
            Date = r.ReceiptDate,
            AmountStr = r.Amount,
